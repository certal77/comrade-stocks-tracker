...
if page=="Analisar ação":
    ...
    with col1:
        ticker = st.text_input("Símbolo / ticker").strip().upper()
    ...
    with col2:
        ...
        st.write("- Introduz ticker e dados do investimento")
        st.write("- Clica em **Obter dados** para análise")
        st.write("- Adiciona ao portfólio se deseja guardar")
    ...
    if st.session_state.data_fetched:
        if st.button("➕ Adicionar ao portfólio"):
            ...
            if storage_backend=="supabase":
                ...
                if ok:
                    st.success("Posição adicionada ao Supabase.")
                    portfolio_df=supabase_load_portfolio()
    ...
    if portfolio_df.empty:
        ...
        for idx,r in portfolio_df.iterrows():
            ...
            if hist is None:
                ...
                total_profit+=profit
            summary_rows.append({"ticker":ticker,"invested":amount,"current_value":current_value,
                                 "profit":profit,"roi":roi})
        st.metric("Total investido", f"€{total_invested:.2f}")
        st.metric("Valor atual total", f"€{total_now:.2f}")
        ...
        for idx,row in portfolio_df.iterrows():
            ...
            with st.expander(f"🔖 {ticker} — Investido €{amount:.2f}"):
                if hist is None:
                    ...
                    c1.download_button("📥 Baixar gráfico", buf.getvalue(), file_name=f"{ticker}_hist.png",
    ...
    if uploaded:
        try:
            ...
            if not expected.issubset(df_new.columns):
